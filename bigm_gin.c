...
Datum gin_extract_query_bigm(PG_FUNCTION_ARGS)
{
	...
	*nentries = (bigm_gin_key_limit == 0) ? bgmlen : Min(bigm_gin_key_limit, bgmlen);
	*pmatch = NULL;
...
Datum gin_bigm_consistent(PG_FUNCTION_ARGS)
{
	...
	case SimilarityStrategyNumber:
		...
		*recheck = bigm_enable_recheck;
		 ...
		 */
		res = (nkeys == 0) ? false : ((((float4)ntrue) / ((float4)nkeys)) >= (float4)bigm_similarity_limit);
		break;
	default:
		elog(ERROR, "unrecognized strategy number: %d", strategy);
...
Datum gin_bigm_triconsistent(PG_FUNCTION_ARGS)
{
	...
	case SimilarityStrategyNumber:
		 ...
		 */
		res = (nkeys == 0) ? GIN_FALSE : (((((float4)ntrue) / ((float4)nkeys)) >= (float4)bigm_similarity_limit) ? GIN_MAYBE : GIN_FALSE);
